...
def process_docx_to_moodle(docx_file_bytes, filename="QUIZ"):
    ...
    with docx2python(docx_file_bytes, html=True) as doc_content:
        ...
        for q in questions:
            ...
            # Perbaikan: Bersihkan tag HTML dari snippet nama
            ...
            # Mendukung Font Arabic & Tabel (dir=auto)
            ...
            if q['type'] == 'multichoice':
                ...
                for i, opt in enumerate(q['options']):
                    # Perbaikan logika deteksi jawaban benar (case-insensitive & support multiple)
        ...
        # 3. FINALISASI (Perbaikan Deklarasi XML Ganda)
        # xml_declaration=False agar kita bisa menambahkan header manual tanpa double declaration
        xml_data = ET.tostring(quiz, encoding='utf-8', method='xml').decode('utf-8')
        ...
        # Hapus deklarasi bawaan ElementTree jika masih muncul
        xml_data = re.sub(r"<\?xml.*?\?>", "", xml_data).strip()
        
        return header_comment + xml_data
